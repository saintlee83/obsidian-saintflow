...
describe("schemaFor", () => {
	it("설계안 2.2의 유형을 모두 덮습니다", () => {
		for (const type of ["task", "project", "area", "resource", "zettel", "map", "working", "output", "recall", "daily", "weekly", "closing"]) {
...
describe("missingRequired (C15 필수 속성)", () => {
	it("Project는 done_criteria이 있어야 합니다", () => {
		assert.deepEqual(
			missingRequired(SCHEMAS.project, { type: "project", status: "active", done_criteria: "" }),
			["done_criteria"]
		);
		assert.deepEqual(
			missingRequired(SCHEMAS.project, { type: "project", status: "active", done_criteria: "끝나는 조건" }),
			[]
		);
	});

	it("Output은 project와 status가 있어야 합니다", () => {
		assert.deepEqual(missingRequired(SCHEMAS.output, { type: "output" }), ["status"]);
	});

	it("Zettel의 box는 recall이 true일 때만 필수입니다", () => {
		assert.deepEqual(
			missingRequired(SCHEMAS.zettel, { type: "zettel", maturity: "seed", recall: false }),
			[]
		);
		assert.deepEqual(
			missingRequired(SCHEMAS.zettel, { type: "zettel", maturity: "seed", recall: true }),
			["box"]
		);
	});
	...
	it("recall이 false면 비어 있어도 필수를 만족합니다", () => {
		assert.deepEqual(
			missingRequired(SCHEMAS.zettel, { type: "zettel", maturity: "evergreen", recall: false, box: null }),
			[]
		);
	});
});
...
describe("valueProblems (C15 값 범위)", () => {
	it("허용값 밖의 status를 잡습니다", () => {
		...
		assert.deepEqual(problems[0].allowed, ["next", "in progress", "waiting", "someday", "done", "dropped"]);
	});
	...
	it("날짜 형식을 봅니다", () => {
		assert.equal(valueProblems(SCHEMAS.task, { type: "task", status: "done", due: "2026-09-21" }).length, 0);
		assert.equal(valueProblems(SCHEMAS.task, { type: "task", status: "done", due: "어제" }).length, 1);
	});
	...
	it("단일 링크 필드에 리스트가 들어오면 잡습니다", () => {
		...
		assert.equal(problems[0].key, "parent");
	});
...
describe("missingKeys / emptyValueFor (C19)", () => {
	it("스키마에 있는데 없는 키를 셉니다", () => {
		...
		assert.deepEqual(keys, ["project", "area", "parent", "scheduled", "due", "waiting_on", "link", "archived", "created"]);
	});

	it("값이 비어 있어도 키가 있으면 누락이 아닙니다 (멱등)", () => {
		const fm = { type: "task", status: "next", project: null, area: null, parent: null, scheduled: null, due: null, waiting_on: null, link: null, archived: false, created: null };
	...
	it("빈 값은 종류에 따라 다릅니다", () => {
		...
		assert.deepEqual(emptyValueFor(fields.source), []);
